...
public class PayeeService {
	...
	public Account addPayee(String accountNumber, Payee newPayee, String confirmAccountNumber) {
		...
		if (payeeAccount == null) {
			throw new ResourceNotFoundException("Payee's account number " + newPayee.getAccountNumber() 
			+ " not found.");
		}

		// Check if the payee already exists for this account
		boolean payeeExists = account.getPayees().stream()
				.anyMatch(existingPayee -> existingPayee.getAccountNumber()
						.equals(newPayee.getAccountNumber()));

		if (payeeExists) {
			throw new IllegalArgumentException("Payee with account number " + newPayee.getAccountNumber() 
